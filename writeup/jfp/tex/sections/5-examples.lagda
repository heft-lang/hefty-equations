...
%% -- -- 
%% -- --     module _ ⦃ u : Univ ⦄ {unit : Ty} ⦃ iso : ⟦ unit ⟧ᵀ ↔ ⊤ ⦄ where
%% -- --       open HeftyModule using (_𝓑_; _>>_)
%% -- \end{code}    
%% -- \begin{code}
...
%% -- %
%% -- \begin{code}[hide]
%% -- --     data CCOp ⦃ u : Univ ⦄ (Ref : Ty → Set) : Set where
%% -- --       sub   : {t : Ty}                           →  CCOp Ref
%% -- --       jump  : {t : Ty} (ref : Ref t) (x : ⟦ t ⟧ᵀ) →  CCOp Ref
%% -- -- 
%% -- --     CC : ⦃ u : Univ ⦄ (Ref : Ty → Set) → Effect
%% -- --     Op  (CC Ref) = CCOp Ref
%% -- --     Ret (CC Ref) (sub {t})         = Ref t ⊎ ⟦ t ⟧ᵀ
%% -- --     Ret (CC Ref) (jump ref x)  = ⊥
%% -- -- 
%% -- --     module _ ⦃ u : Univ ⦄ {Ref : Ty → Set} {t : Ty} ⦃ w : Δ ∼ CC Ref ▸ Δ′ ⦄ where
%% -- \end{code}
%% -- \begin{code}
...
%% -- \end{code}
%% -- \begin{code}[hide]
%% -- --       data OnceOp ⦃ u : Univ ⦄ : Set where once : {t : Ty} → OnceOp
%% -- -- 
%% -- --       Once : ⦃ u : Univ ⦄ → Effectᴴ
%% -- --       Opᴴ    Once          = OnceOp
...
%% -- -- 
%% -- --       interleaveₗ : ⦃ u : Univ ⦄ {Ref : Ty → Set} {-⦃ w : Δ ∼ Choice ▸ Δ′ ⦄-}
